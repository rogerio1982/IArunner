...
- **Front-end**: HTML + CSS (Tailwind via CDN) + JavaScript puro, sem
  framework nem build tool. Arquivos estáticos em `public/`.
...
1. **Landing page** (`public/index.html`) — apresenta o produto, com CTAs
   para criar conta ou entrar.
2. **Cadastro** (`public/cadastro.html` → `api/auth/cadastro.php`) — nome,
   e-mail, WhatsApp e senha. Cria o usuário com **7 dias de teste grátis**
   (`trial_ends_at`) e já inicia a sessão.
3. **Onboarding** (`public/onboarding.html` → `api/onboarding.php`) — usuário
...
4. **Dashboard** (`public/dashboard.html` → `api/dashboard.php`) — tela
   principal, mostra:
...
6. **Assinatura** (`public/pagamento.html` → `api/pagamento.php`) — cria uma
   preferência de pagamento no Mercado Pago (Checkout Pro) e redireciona o
   usuário para a página de pagamento deles.
7. **Retorno do pagamento** (`public/pagamento_retorno.html` →
...
- **Importação de treinos** (`public/admin/import.html` →
...
```
public/              → front-end estático (HTML + JS), servido ao navegador
...
includes/            → lógica compartilhada usada pela API
...
config/database.php  → conexão PDO + leitura de variáveis de ambiente (.env)

webhook.php           → recebe notificações de pagamento do Mercado Pago
router.php            → roteador usado só pelo servidor PHP embutido (dev local)
...
- Para rodar localmente: servidor PHP embutido com `public/` como docroot e
  `router.php` roteando `/api/*` e `/webhook.php` para a raiz do projeto:
  ```
